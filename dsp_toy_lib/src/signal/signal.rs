    ...
    pub fn iir_filter_peak_bell_imag(&self, bands : &[(f64, f64, f64)], sample_rate : usize) -> Self {
        ...
        let filter = |inps : &[Complex64], outs : &[Complex64], b_coeffs : &[Complex64], a_coeffs : &[Complex64]| -> Complex64 {
            let tmp_b : Complex64 = b_coeffs
                .iter()
                .enumerate()
                .map(|(i, val)| *val * inps[inps.len() - 1 - i])
                .reduce(|sum, x| sum + x)
                .unwrap();
        ...
        let max_prev_inps = feedforward_coeffs_norm.len();
        let max_prev_outs = feedback_coeffs_norm.len();
        let mut inps = VecDeque::from(vec![Complex64::from(0.); max_prev_inps]);
        let mut outs = VecDeque::from(vec![Complex64::from(0.); max_prev_outs]);
        for val in self.data.iter().map(|x| *x) {
            inps.pop_front();
            inps.push_back(val);
