...
// This is for when T is integer or floating point. Not Complex. Later, an int-only version can be made, which would in theory have a better performance than this.
...
// TODO implement Linear Kalman for Complex

impl<T, KalmanInput, const STATE_DIM : usize, const MEASURE_DIM : usize, const CONTROL_DIM : usize> RealTimeSignalProcessor<KalmanInput, Complex<T>> for FilterKalmanLinear<Complex<T>, STATE_DIM, MEASURE_DIM, CONTROL_DIM>
where 
    T: RealField + Copy + Clone
{
    fn process_sample(&mut self, inp : KalmanInput) -> Complex<T> {
        todo!()
    }
}